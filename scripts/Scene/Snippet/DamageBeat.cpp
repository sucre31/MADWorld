...
bool DamageBeat::update() {
	calcDamagePos();
	if (snippetGameManager->getFpsIns()->isFrameChanged() && ShowDamageHit != 0) {
		ShowDamageHit++;
		if (ShowDamageHit == 7) {
			ShowDamageHit = 0;
		}
...
void DamageBeat::draw() const {
	int i;
	if (ShowDamageHit > 0) {
		DrawGraph(DamageHitPosX, DamageHitPosY, snippetImage->getHitImage()[((ShowDamageHit - 1) / 3) % 2], TRUE);
	}
	for (i = 0; i < numberOfDamage; i++) {
		if (damageNumber[i]->damage < 100) {
			if (damageNumber[i]->damage < 10) {		//ダメージが1桁
				DrawGraph((int)damageNumber[i]->x + 160, (int)damageNumber[i]->y, snippetImage->getBattleCharacter()[damageNumber[i]->damage], TRUE);
			}
			else {									//ダメージが2桁
				DrawGraph((int)damageNumber[i]->x + 151, (int)damageNumber[i]->y, snippetImage->getBattleCharacter()[(damageNumber[i]->damage / 10) % 10], TRUE);
				DrawGraph((int)damageNumber[i]->x + 160, (int)damageNumber[i]->y, snippetImage->getBattleCharacter()[(damageNumber[i]->damage % 10)], TRUE);
			}
		}
		else {										//ダメージが3桁
			DrawGraph((int)damageNumber[i]->x + 142, (int)damageNumber[i]->y, snippetImage->getBattleCharacter()[(damageNumber[i]->damage / 100) % 10], TRUE);
			DrawGraph((int)damageNumber[i]->x + 151, (int)damageNumber[i]->y, snippetImage->getBattleCharacter()[(damageNumber[i]->damage / 10) % 10], TRUE);
			DrawGraph((int)damageNumber[i]->x + 160, (int)damageNumber[i]->y, snippetImage->getBattleCharacter()[damageNumber[i]->damage % 10], TRUE);
		}
	}
