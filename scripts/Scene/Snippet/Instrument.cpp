...
Instrument::Instrument(std::shared_ptr<SnippetSound> sound) {
	snippetSound = sound;
}
...
void Instrument::playMainInstrument(int NumberOfTone) {
	...
	case 0:
		...
		PlaySoundMem(snippetSound->getLucasBattleSounds()[targetPlayNum], DX_PLAYTYPE_BACK);
		break;
	case 1:
		...
		if (NumberOfTone % 6 != 5) {
			...
			PlaySoundMem(snippetSound->getKumatoraBattleSounds()[targetPlayNum], DX_PLAYTYPE_BACK);
		}
		else {
	...
	case 2:
		...
		if (NumberOfTone < 62) {
			...
			if (NumberOfTone % 2 == 0) {
				targetPlayNum = targetPlayNum * 2;
				PlaySoundMem(snippetSound->getDusterBattleSounds()[targetPlayNum], DX_PLAYTYPE_BACK);
			}
			else {
				targetPlayNum = targetPlayNum * 2 + 1;
				PlaySoundMem(snippetSound->getDusterBattleSounds()[targetPlayNum], DX_PLAYTYPE_BACK);
			...
			if (NumberOfTone == 62) {
				PlaySoundMem(snippetSound->getDusterBattleSounds()[10], DX_PLAYTYPE_BACK);
			}
			else if (NumberOfTone == 63) {
				PlaySoundMem(snippetSound->getDusterBattleSounds()[11], DX_PLAYTYPE_BACK);
			}
			else if (NumberOfTone == 64) {
				PlaySoundMem(snippetSound->getDusterBattleSounds()[12], DX_PLAYTYPE_BACK);
			}
			else if (NumberOfTone == 65) {
				PlaySoundMem(snippetSound->getDusterBattleSounds()[13], DX_PLAYTYPE_BACK);
			}
		}
		break;
	case 3:
		...
		PlaySoundMem(snippetSound->getBoneyBattleSounds()[targetPlayNum], DX_PLAYTYPE_BACK);
		break;
	}
}
