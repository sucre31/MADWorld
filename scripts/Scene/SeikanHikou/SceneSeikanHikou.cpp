...
SceneSeikanHikou::SceneSeikanHikou(IOnSceneChangedListener* impl, const Parameter& parameter)
	...
	std::string rankaFile = "Assets/Sounds/SeikanHikou/ranka";
	std::string rankaFileAvi = "Assets/Sprites/movie/ranka/mp4Mute/ranka";
	for (int i = 1; i < 25; i++) {
		if (i < 10) {
			sozaiManager.makeSozai((rankaFile + std::to_string(i - 1) + ".wav").c_str(), (rankaFileAvi + "0" + std::to_string(i) + ".mp4").c_str(), 640, 360);
		}
		else {
			sozaiManager.makeSozai((rankaFile + std::to_string(i - 1) + ".wav").c_str(), (rankaFileAvi + std::to_string(i) + ".mp4").c_str(), 640, 360);
		}
